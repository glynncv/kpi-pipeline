...
class OKRCalculator:
    ...
    def calculate_kr_score(self, kr_id: str) -> Dict[str, Any]:
        ...
        # Try with /KR suffix first, then without
        kpi_key = f"{kpi_id}/KR{kr_id[2]}" if kpi_id != 'SM001' else kpi_id
        kpi_result = self.kpi_results.get(kpi_key, self.kpi_results.get(kpi_id))
        
        if not kpi_result:
            logger.warning(f"KPI {kpi_id} not found for {kr_id}")
            return {
                'kr_id': kr_id,
                'name': kr_config['name'],
                'score': 0,
    ...
    def _extract_current_value(self, kpi_id: str, kpi_result: Dict) -> float:
        ...
        Args:
            kpi_id: KPI identifier ('SM002', 'SM003', 'SM004')
            kpi_result: KPI calculation result dictionary
        
        Returns:
            Current value for the KR metric
        """
        if kpi_id == 'SM002':  # KR4 - Incident Backlog
            # SM002 calculates Backlog_Percentage
            return kpi_result.get('Backlog_Percentage', 0.0)
        
        elif kpi_id == 'SM003':  # KR5 - Request Backlog
            # SM003 calculates Aged_Percentage
            return kpi_result.get('Aged_Percentage', 0.0)
        
        elif kpi_id == 'SM004':  # KR6 - First Time Fix
            # SM004 calculates FCR_Percentage
            return kpi_result.get('FCR_Percentage', 0.0)
    ...
    def get_action_triggers(self) -> Dict[str, list]:
        ...
        if kr6_triggers.get('critical'):
            # Use ftf_rate key (which maps to FCR_Percentage from KPI)
            fcr_percentage = kr6.get('current_value', 0)
            if fcr_percentage < 50:
                triggers['critical'].append({
                    'kr_id': 'KR6',
