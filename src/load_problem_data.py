...
def load_problem_data(data_dir: str = 'data', config: Optional[Dict[str, Any]] = None) -> Optional[pd.DataFrame]:
    """
    ...
    problem_filename = config_loader.get_problem_filename(config)
    problem_file = Path(data_dir) / problem_filename
    
    if not problem_file.exists():
        print(f"[WARN] Warning: Problem file not found: {problem_file}")
    ...
    try:
        ...
        print(f"[OK] Loaded {len(df)} problems from {problem_filename}")
        
        # Basic validation
        required_cols = ['number', 'opened_at', 'priority']
        missing_cols = [col for col in required_cols if col not in df.columns]
        if missing_cols:
            print(f"[WARN] Warning: Missing required columns: {missing_cols}")
        
        return df
        
    except Exception as e:
        print(f"[FAIL] Error loading problem data: {e}")
        return None


def load_task_data(data_dir: str = 'data', config: Optional[Dict[str, Any]] = None) -> Optional[pd.DataFrame]:
    """
    ...
    task_filename = config_loader.get_task_filename(config)
    task_file = Path(data_dir) / task_filename
    
    if not task_file.exists():
        print(f"[WARN] Warning: Task file not found: {task_file}")
    ...
    try:
        ...
        print(f"[OK] Loaded {len(df)} RCA tasks from {task_filename}")
        ...
        print(f"[FAIL] Error loading task data: {e}")
        return None


def load_all_problem_data(data_dir: str = 'data/input', config: Optional[Dict[str, Any]] = None) -> Tuple[Optional[pd.DataFrame], Optional[pd.DataFrame]]:
    """
    ...
    if problems is None or tasks is None:
        print("\n[WARN] Problem Management data incomplete - PM KPIs will be skipped")
        print("=" * 80)
        return None, None
    ...
    if len(matching) < len(problems) * 0.5:
        print(f"\n[WARN] Warning: Low task coverage (<50%)")
...
def test_load_problem_data():
    ...
    if problems is not None and tasks is not None:
        print("\n[OK] Both files loaded successfully!")
        ...
        if missing_problem:
            print(f"  [WARN] Missing problem fields: {missing_problem}")
        else:
            print(f"  [OK] All required problem fields present")
        ...
        if missing_task:
            print(f"  [WARN] Missing task fields: {missing_task}")
        else:
            print(f"  [OK] All required task fields present")
        ...
        print("\n[OK] Data loading tests complete!")
        
    else:
        print("\n[WARN] Problem Management data not available")
